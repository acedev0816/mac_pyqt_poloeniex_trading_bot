...
def maPlot(st, et,cur,dp):
	...
	for a in range(size):
		...
		dates1 = datetime.datetime.fromtimestamp(int(dates)).strftime('%d')
	...
	for a in range(countofMA): #First Loop i indicating moving average index 
		...
		plotPrices.append(cPricesPlot[index])
		plotDates.append(datesConv[index])
	figure = plt.figure()
	
	plt.plot(plotDates, plotPrices)
	plt.plot(plotDates, movingAvgs)
	plt.title("MA Plot")
	plt.show()
	# ret = []	
	...
	# 	ret.append(one)
	# return ret
# maPlot('2019/4/1','2019/4/20','BTC_ETH',11)
